...
description: >
  ...
  action plans, decisions, open questions, and task lists. Organize information
  clearly without inventing, assuming, or filling gaps. Preserve all names,
  dates, numbers, and terminology exactly as provided.
version: 1.0.0
author: sidharth0018
modes:
  - Full Breakdown
...
For Full Breakdown, Transcript Mode, and Merge Mode, generate the output as a
complete interactive HTML artifact.
...
| Rule | Description |
|---|---|
| No invention | If data is missing, output `Not specified` — never guess |
| Surface conflicts | Flag both conflicting versions — never silently resolve |
| Preserve attribution | Keep speaker/source labels exactly as written |
| Completed ≠ Pending | Items marked ✓ done are NOT re-added as action items |
| Ideas ≠ Tasks | Unconfirmed ideas go to Open Questions, not Action Items |
...
Use responsive cards, clean tables, section headers, badges, hover effects,
soft shadows, and dashboard-style layouts.
...
For Transcript Mode:
```
/brain-dump-action-planner transcript-mode
...
For Merge Mode:
```
/brain-dump-action-planner merge-mode
